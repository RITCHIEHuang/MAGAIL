...
class Actor(nn.Module):
    def __init__(self, num_states, num_actions, num_discrete_actions=0, discrete_actions_sections: Tuple = (0,),
                 action_log_std=0, num_hiddens: Tuple = (64, 64), activation: nn.Module = nn.LeakyReLU,
                 drop_rate=None):
        """
        ...
        if self.num_discrete_actions:
            self._module_list.append(MultiSoftMax(0, self.num_discrete_actions, self.discrete_action_sections))
    ...
    def get_action_log_prob(self, states):
        ...
        log_prob = dist_continuous.log_prob(action).sum(dim=-1)  # [1, states.shape[0]]
        if dist_discrete:
            discrete_action = dist_discrete.sample()
            discrete_log_prob = dist_discrete.log_prob(discrete_action)  # [1, states.shape[0]]

            action = torch.cat([discrete_action, action], dim=-1)
            log_prob = (log_prob + discrete_log_prob)

        return action, log_prob.unsqueeze(-1)  # [states.shape[0], 1]
